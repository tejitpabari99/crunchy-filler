...
async function fetchFanFavouriteEpisodes(animeTitle) {
  try {
    const response = await fetch(browser.runtime.getURL('data/fanFavourites.json'));
        ...
        browser.runtime.sendMessage(
          ...
          (badgeResponse) => {
            if (browser.runtime.lastError) {
              console.error("Badge update error:", browser.runtime.lastError);
            }
          }
        );
      }
        ...
        try {
          if (browser.runtime && browser.runtime.sendMessage) {
            browser.runtime.sendMessage(
              ...
              async (response) => {
                if (browser.runtime.lastError) {
                  console.error("SendMessage Error:", browser.runtime.lastError);
                  updateBadgeWithCount(0);
                } else {
                  let fillerCount = 0;
            ...
            console.warn("Browser runtime not available, cannot fetch filler episodes");
            updateBadgeWithCount(0);
          }
        } catch (error) {
